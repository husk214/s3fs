#ifndef FSSS_ELASTIC_SOFT_INSENSITIVE_H_
#define FSSS_ELASTIC_SOFT_INSENSITIVE_H_

#include "sdm/lib/utils.hpp"
#include "sdm/lib/supervised_data_regression.hpp"
...
class elastic_soft_insensitive
    : public sdm::supervised_data_regression<double, 1> {
public:
  elastic_soft_insensitive(const std::string &libsvm_format,
                           const double &lam = 1.0, const double &eps = 1.0,
                           const double &gamma = 0.5,
                           const double &stop_cri = 1e-3,
                           const int &max_iter = 1000, const int &fcg = 1,
                           const double &fsfs3 = 1e-1);
  ~elastic_soft_insensitive();
  ...
  void set_frequency_cal_gap_(const int &fcg);
  ...
  // for Simultaneous Feature and Sample Safe Screening (SFS3)
