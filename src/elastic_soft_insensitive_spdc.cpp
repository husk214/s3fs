...
elastic_soft_insensitive_spdc::elastic_soft_insensitive_spdc(
...
void elastic_soft_insensitive_spdc::set_regularized_parameter(
    const double &lam) {
  lambda_ = lam;
  tau_ = std::sqrt(gamma_ / (lambda_ * num_ins_)) / R_;
  sigma_ = std::sqrt((num_ins_ * lambda_) / gamma_) / R_;
...
void elastic_soft_insensitive_spdc::init_za_over_n(void) {
...
void elastic_soft_insensitive_spdc::calculate_dual_obj_value(
...
void elastic_soft_insensitive_spdc::calculate_duality_gap(
...
void elastic_soft_insensitive_spdc::train(void) {
  int i = 0;
  double yi = 0.0;
  ...
  const double sig_gam = 1.0 / (sigma_ * gamma_ + 1.0);
  const double taulam_one = tau_lam + 1.0;
  const double sig_eps = sigma_ * epsilon_;
  ...
  for (itr_ = 1; itr_ < max_iter_ && duality_gap_ > stop_criterion_; ++itr_) {
    for (int ir = 0; ir < num_ins_; ++ir) {
      ...
      yi = y_[i];
      double beta_sign =
          sigma_ * (-(x_.row(i) * spdc_w_)(0) + yi) + dual_var_[i];
      if (beta_sign > sig_eps) {
        // beta > 0
        alpha_i_new = std::min(1.0, (beta_sign - sig_eps) * sig_gam);
      } else if (beta_sign < -sig_eps) {
        // beta < 0
        alpha_i_new = std::max(-1.0, (beta_sign + sig_eps) * sig_gam);
      } else {
        alpha_i_new = 0.0;
      }
      ...
      delta_v = delta_alpha_i * x_.row(i);
...
void elastic_soft_insensitive_spdc::train_sfs3(const bool dynamic) {
  int i = 0;
  double yi = 0.0;
  double delta_alpha_i = 0.0;
  double alpha_i_new = 0.0;
...
void elastic_soft_insensitive_spdc::train_fs(const bool dynamic) {
  int i = 0;
  double yi = 0.0;
  double delta_alpha_i = 0.0;
  double alpha_i_new = 0.0;
...
void elastic_soft_insensitive_spdc::train_ss(const bool dynamic) {
  int i = 0;
  double yi = 0.0;
  double delta_alpha_i = 0.0;
  double alpha_i_new = 0.0;
...
void elastic_soft_insensitive_spdc::train_for_plot_inverse(void) {
