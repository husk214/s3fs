...
elastic_smooth_hinge::elastic_smooth_hinge(const std::string &libsvm_format,
                                           const double &lam,
                                           const double &gamma,
                                           const double &stop_cri,
                                           const int &max_iter, const int &fcg,
                                           const double &fsfs3)
    : supervised_data<double, 1>(libsvm_format), lambda_(lam), gamma_(gamma),
      stop_criterion_(stop_cri), itr_(0), max_iter_(max_iter),
      frequency_cal_gap_(fcg), frequency_sfs3_(fsfs3), R_(0.0),
      primal_obj_value_(0.0), dual_obj_value_(0.0), cm_x_(x_) {
  ...
  fs_start_gap_ = 1.0;
  ss_start_gap_ = 1e-2;
  screening_end_rate_ = 0.99;
  flag_fs_end_ = false;
  flag_ss_end_ = false;
}

elastic_smooth_hinge::~elastic_smooth_hinge() {}

int elastic_smooth_hinge::get_num_ins(void) const { return num_ins_; }

int elastic_smooth_hinge::get_num_fea(void) const { return num_fea_; }

double elastic_smooth_hinge::get_regularized_parameter(void) const {
  return lambda_;
}

double elastic_smooth_hinge::get_min_regularized_parameter(void) const {
  return lambda_max_;
}

double elastic_smooth_hinge::get_primal_obj_value(void) const {
  return primal_obj_value_;
}

double elastic_smooth_hinge::get_dual_obj_value(void) const {
  return dual_obj_value_;
}

double elastic_smooth_hinge::get_duality_gap(void) const {
  return duality_gap_;
}

double elastic_smooth_hinge::get_stop_criterion(void) const {
  return stop_criterion_;
}

void elastic_smooth_hinge::set_regularized_parameter(const double &lam) {
  lambda_ = lam;
  duality_gap_ = std::numeric_limits<double>::max();
  itr_ = 0;
}

void elastic_smooth_hinge::set_stop_criterion(const double &eps) {
  stop_criterion_ = eps;
}

void elastic_smooth_hinge::set_ssfss_parameter(const int &msi, const int &emsi,
                                               const double &fsg,
                                               const double &ssg,
                                               const double &ser) {
  max_ssfss_itr_ = msi;
...
void elastic_smooth_hinge::calculate_primal_obj_value(
    const bool &flag_cal_loss) {
  sum_loss_ = 0.0;
  if (flag_cal_loss) {
    b_ = (1.0 - y_ * (x_ * primal_var_).array()).matrix();
    double bi;
    for (int i = 0; i < num_ins_; ++i) {
      bi = b_[i];
      if (bi > gamma_) {
...
void elastic_smooth_hinge::calculate_dual_obj_value(const bool &flag_cal_v) {
  if (flag_cal_v) {
    za_over_n_ = Eigen::VectorXd::Zero(num_fea_);
    for (int i = 0; i < num_ins_; ++i)
      za_over_n_ += dual_var_[i] * y_[i] * x_.row(i);
...
void elastic_smooth_hinge::calculate_duality_gap(const bool &flag_cal_loss,
...
Eigen::VectorXd elastic_smooth_hinge::get_primal_var(void) const {
  return primal_var_;
}

Eigen::VectorXd elastic_smooth_hinge::get_dual_var(void) const {
  return dual_var_;
}

void elastic_smooth_hinge::claer_sfs3_index(void) {
  w_nnz_index_.clear();
  w_nnz_index_.insert(std::begin(all_fea_index_), std::end(all_fea_index_));
  fea_screen_target_.clear();
  fea_screen_target_.insert(std::begin(all_fea_index_),
...
void elastic_smooth_hinge::sfs3(const bool flag_init) {
  ...
  while (w_nnz_size_old != w_nnz_size || non_scr_size != non_scr_size_old) {
    ...
    if ((num_fea_ - w_nnz_size + num_total_fea_sav_) >
...
void elastic_smooth_hinge::safe_feature_screen(const bool flag_only) {
  ...
  double max_abs_zja;
  bool flag_ss1_fill = !(sam_screening_index_.second.empty());
  ...
  for (auto &&j : fea_screen_target_) {
    ...
    double established_term = 0.0;
    if (flag_ss1_fill)
      established_term = (established_dual_var_.transpose() * cm_x_.col(j))(0);
    double unestablished_term =
        (unestablished_dual_var_.transpose() * cm_x_.col(j))(0);
    ...
    double first_half = established_term + unestablished_term;
...
void elastic_smooth_hinge::initial_safe_feature_screen(const bool flag_only) {
  auto start_time = sys_clk::now();
  double dual_radius =
      std::sqrt(std::max(0.0, 2.0 * num_ins_ * duality_gap_ / gamma_));
...
void elastic_smooth_hinge::safe_sample_screen(const bool flag_only) {
  auto start_time = sys_clk::now();
  double primal_radius = std::sqrt(std::max(0.0, 2.0 * duality_gap_ / lambda_));
  double dual_radius =
      std::sqrt(std::max(0.0, (2.0 * num_ins_ * duality_gap_ / gamma_)));

  std::vector<int> new_nsv0, new_nsv1, new_sav;
  double unestablished_term, tmp, lb_zuwu, ub_zuwu;
  for (auto &&i : sam_screen_target_) {
    bool flag_saving = false;
    if (dual_var_[i] > dual_radius && dual_var_[i] < 1.0 - dual_radius) {
      ++num_inv_sam_scr_;
      ++num_total_ins_sav_;
      flag_saving = true;
      new_sav.push_back(i);
    }
    unestablished_term = y_[i] * (x_.row(i) * primal_var_)(0);
    ...
    } else if (ub_zuwu <= 1.0 - gamma_) {
      new_nsv1.push_back(i);
    } else if (1 - gamma_ < lb_zuwu && ub_zuwu < 1) {
      ++num_ins_sav_;
      if (!flag_saving) {
        ++num_total_ins_sav_;
        new_sav.push_back(i);
  ...
  for (auto &&i : new_nsv0) {
    ...
    dvi = dual_var_[i];
    if (dvi > 0.0) {
      za_over_n_ -= one_over_n_ * dvi * y_[i] * x_.row(i);
...
void elastic_smooth_hinge::initial_safe_sample_screen(const bool flag_only) {
  auto start_time = sys_clk::now();
  double primal_radius = std::sqrt(2.0 * duality_gap_ / lambda_);
  double dual_radius =
      std::sqrt(std::max(0.0, 2.0 * num_ins_ * duality_gap_ / gamma_));
  ...
  Eigen::ArrayXd lbzw = y_ * (x_ * primal_var_).array();
  ...
  for (int i = 0; i < num_ins_; ++i) {
    bool flag_saving = false;
    dvi = dual_var_[i];
    if (dvi > dual_radius && dvi < 1.0 - dual_radius) {
      ++num_inv_sam_scr_;
      ++num_total_ins_sav_;
      flag_saving = true;
      sam_screen_target_.erase(i);
    ...
    if (lbzw[i] >= 1.0) {
      ...
      unestablished_dual_var_[i] = 0.0;
      unestablished_sam_flag_[i] = 0.0;
    } else if (1 - gamma_ < lbzw[i] && ubzw[i] < 1) {
      ++num_ins_sav_;
      if (!flag_saving) {
        ++num_total_ins_sav_;
        sam_screen_target_.erase(i);
...
void elastic_smooth_hinge::inverse_screen(void) {
  num_inv_fea_scr_ = 0;
  num_inv_sam_scr_ = 0;
  double primal_radius = std::sqrt(std::max(0.0, 2.0 * duality_gap_ / lambda_));
  double dual_radius =
...
void elastic_smooth_hinge::further_inverse_screen(void) {
  double primal_radius = std::sqrt(std::max(0.0, 2.0 * duality_gap_ / lambda_));
  double dual_radius =
      std::sqrt(std::max(0.0, 2.0 * num_ins_ * duality_gap_ / gamma_));
...
std::vector<int> elastic_smooth_hinge::screening_for_plot_inverse(void) {
